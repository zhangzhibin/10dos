...
/**
 * Mock task definition (text + age; optional completed)
 */
...
class TodoApp {
   ...
   */
  private loadMockGroup(groupIndex: number): void {
  ...
  private readonly LIST_SLOTS = 10; // 列表始终显示的位置数
  ...
  private render(): void {
    ...
    const emptySlotsHtml = Array.from(
      { length: emptySlotsCount },
      (_, i) => `<div class="todo-item todo-item-empty" aria-hidden="true"><input type="checkbox" class="todo-checkbox todo-checkbox-empty" disabled><span class="todo-number">${sortedTodos.length + i + 1}</span><div class="todo-content"></div></div>`
    ).join('');
